...
class BackendCircuitAQT(BackendCircuitQiskit):
    ...
    def do_simulate(self, variables, initial_state=0, *args, **kwargs) -> QubitWaveFunction:
        ...
        if type(qiskit_backend) == qiskit_aqt_provider.aqt_resource.OfflineSimulatorResource:
            ...
            print(backend_result.quasi_dists[0])
            ...
            wavefunction._state = backend_result.results[0].data.counts  # todo make initializer
            print(wavefunction)
            print(backend_result.results[0].data.counts) # make this either to integer or see if it can read statevector
        return wavefunction
